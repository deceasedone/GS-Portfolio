...
export function TextRoll({
  ...
  // If not triggered yet (e.g., not in view), render static text preserving styles
  if (!trigger) {
    ...
    <span className={className}>
      {letters.map((letter, i) => {
        return (
          ...
          >
            <motion.span
              className={`absolute inline-block [backface-visibility:hidden] [transform-origin:50%_25%] ${textClassName ?? ''}`}
              initial={
            ...
            <motion.span
              className={`absolute inline-block [backface-visibility:hidden] [transform-origin:50%_100%] ${textClassName ?? ''}`}
